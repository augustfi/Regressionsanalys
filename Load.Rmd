...
# Fit all possible subsets models
bodyfat.model <- lm(density ~ age + weight + height + neck + chest + abdomen +
                    hip + thigh + knee + ankle + biceps + forearm + wrist,
                    data = bodyfatmen)
...
cat("Best model by adjusted R^2 has", best.adjr2, "predictors\n")
cat("Best model by Mallows' Cp has", best.cp, "predictors\n")
cat("Best model by BIC has", best.bic, "predictors\n")
...
```
```{r Adjusted R2, Mallows Cp, BIC, AIC}
# Load necessary package
#if (!require(leaps)) install.packages("leaps")
library(leaps)

# Run all subsets regression with up to 13 predictors
all.subsets <- regsubsets(density ~ age + weight + height + neck + chest + abdomen +
                          hip + thigh + knee + ankle + biceps + forearm + wrist,
                          data = bodyfatmen, nvmax = 13)

# Get summary of the regsubsets output
all.subsets.summary <- summary(all.subsets)

# Identify best model indices for each criterion
best.adjr2_index <- which.max(all.subsets.summary$adjr2)
best.cp_index    <- which.min(all.subsets.summary$cp)
best.bic_index   <- which.min(all.subsets.summary$bic)

cat("Best model by Adjusted R^2 uses", best.adjr2_index, "predictors\n")
cat("Best model by Mallows' Cp uses", best.cp_index, "predictors\n")
cat("Best model by BIC uses", best.bic_index, "predictors\n")

# Compute AIC for each model size
# AIC = n*log(RSS/n) + 2K, where K = number of parameters (predictors + intercept)
n <- nrow(bodyfatmen)
num_models <- length(all.subsets.summary$rss)  # should be 13
aic_values <- rep(NA, num_models)

for(i in 1:num_models) {
  k <- i + 1  # i predictors + intercept
  rss <- all.subsets.summary$rss[i]
  aic_values[i] <- n * log(rss / n) + 2 * k
}

best.aic_index <- which.min(aic_values)
cat("Best model by AIC uses", best.aic_index, "predictors\n")

# Extract coefficients for the best models according to each criterion
best_model_adjr2 <- coef(all.subsets, best.adjr2_index)
best_model_cp    <- coef(all.subsets, best.cp_index)
best_model_bic   <- coef(all.subsets, best.bic_index)
best_model_aic   <- coef(all.subsets, best.aic_index)
...
ggpairs(data = data.frame(bodyfatmen[,-1]))
