...
resplot <- function(obj, plots = 1:4) {
    ...
    if (1 %in% plots) {
        plot(fitted(obj), resid(obj),
             pch = 20, xlab = "Fitted Values", ylab = "Residuals")
        title("Tukey-Anscombe-Plot with Resampling")
        for (i in 1:100) {
            lines(loess.smooth(fitted(obj), sample(resid(obj, replace = TRUE))),
                  col = "grey")
        }
        abline(h = 0, lty = 2)
        points(fitted(obj), resid(obj), pch = 20);
        box()
    ...
    if (2 %in% plots) {
        ...
        for (i in 1:100) {
            lines(sort(qq$x),
                  sort(rnorm(length(qq$y), mean(qq$y), sd(qq$y))), col = "grey")
        }
    ...
    if (3 %in% plots) {
        plot(fitted(obj), sqrt(abs(rstandard(obj))),
             pch = 20,
        ...
        for (i in 1:100) {
            lines(loess.smooth(fitted(obj), sample(sqrt(abs(rstandard(obj))),
                                                   replace = TRUE)), col = "grey")
        }
        points(fitted(obj), sqrt(abs(rstandard(obj))), pch = 20);
        box()
